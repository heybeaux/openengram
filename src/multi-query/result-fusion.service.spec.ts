...
describe('ResultFusionService', () => {
  ...
  describe('fuse', () => {
    it('should route to correct strategy', () => {
      ...
      expect(() =>
        service.fuse(searchResults, FusionStrategy.RRF),
      ).not.toThrow();
      expect(() =>
        service.fuse(searchResults, FusionStrategy.FREQUENCY),
      ).not.toThrow();
      expect(() =>
        service.fuse(searchResults, FusionStrategy.WEIGHTED),
      ).not.toThrow();
      expect(() =>
        service.fuse(searchResults, FusionStrategy.MAX_SCORE),
      ).not.toThrow();
    });
  });
  ...
  describe('fuseWithFrequency', () => {
    it('should boost memories appearing in more queries', () => {
      const searchResults = createSearchResults([
        ...
        { query: 'q3', queryIndex: 2, matches: [{ id: 'A', score: 0.72 }] },
      ]);
  ...
  describe('fuseWithMaxScore', () => {
    it('should use maximum similarity score', () => {
      const searchResults = createSearchResults([
        { query: 'q1', queryIndex: 0, matches: [{ id: 'A', score: 0.7 }] },
        { query: 'q2', queryIndex: 1, matches: [{ id: 'A', score: 0.95 }] },
      ]);

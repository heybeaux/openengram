...
import { ImportExecutionService, BULK_IMPORT_QUEUE, BULK_IMPORT_JOB } from './import-execution.service';
  ...
  async process(job: Job<BulkImportJobData>): Promise<void> {
    switch (job.name) {
      case BULK_IMPORT_JOB:
        this.logger.log(`Processing bulk import job: ${job.id} (jobId=${job.data.jobId})`);

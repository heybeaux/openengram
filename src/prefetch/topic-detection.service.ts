...
import { KEYWORD_RULES, getTopicDefinition, getRelatedTopics, TOPIC_DEFINITIONS } from './topic-taxonomy';
...
export class TopicDetectionService {
  ...
  async initializePrototypes(): Promise<void> {
    ...
    for (const topic of TOPIC_DEFINITIONS) {
      try {
        const embedding = await this.embeddingService.generate(topic.prototypeQuery);
        ...
        console.warn(`Failed to initialize prototype for topic ${topic.id}:`, error);
      }
    }
  }
   ...
   */
  setPrototype(topicId: TopicId, embedding: number[], threshold: number = 0.5): void {
  ...
  async detect(
    ...
    if (this.config.enableEmbeddingClassification && this.embeddingService && this.prototypes.size > 0) {
  ...
  detectTopicShift(
    ...
    for (const [topic, avgConfidence] of previousTopics) {
      const currentScore = currentTopics.find(t => t.topic === topic)?.confidence || 0;
    ...
    for (const rule of KEYWORD_RULES) {
      ...
      if (matchCount > 0) {
        ...
        if (topicDef?.parentId) {
          ...
          scores.set(topicDef.parentId, Math.max(parentScore, confidence * 0.7));
        }
      }
    }
   ...
   */
  private async classifyByEmbedding(message: string): Promise<Map<TopicId, number>> {
    const scores = new Map<TopicId, number>();
    ...
    try {
      ...
      for (const [topicId, proto] of this.prototypes) {
        const similarity = this.cosineSimilarity(messageEmbedding, proto.embedding);
  ...
  private mergeScores(
    ...
    const totalWeight = this.config.layerWeights.keyword + this.config.layerWeights.embedding;
    ...
    for (const topic of allTopics) {
      ...
      if (this.config.layerWeights.embedding === 0 || embeddingScores.size === 0) {
        ...
        mergedScore = 
          (keywordScore * this.config.layerWeights.keyword +
           embeddingScore * this.config.layerWeights.embedding) / normalizer;
      }

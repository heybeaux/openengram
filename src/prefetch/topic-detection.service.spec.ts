...
import { TopicDetectionService, DEFAULT_DETECTION_CONFIG } from './topic-detection.service';
...
describe('TopicDetectionService', () => {
  ...
  describe('keyword detection', () => {
    it('should detect family topic from "wife" keyword', async () => {
      ...
      expect(result.topics.some(t => t.topic === 'family' || t.topic === 'family/immediate')).toBe(true);
    });
    ...
    it('should detect schedule topic from "today" keyword', async () => {
      ...
      expect(result.topics.some(t => t.topic.startsWith('schedule'))).toBe(true);
    });

    it('should detect technical topic from programming keywords', async () => {
      const result = await service.detect('How do I fix this TypeScript error?');
      expect(result.topics.some(t => t.topic === 'technical')).toBe(true);
    });

    it('should detect health topic from exercise keywords', async () => {
      const result = await service.detect('I need to go to the gym for my workout');
      expect(result.topics.some(t => t.topic.startsWith('health'))).toBe(true);
    });
    ...
    it('should detect preferences topic from "favorite" keyword', async () => {
      ...
      expect(result.topics.some(t => t.topic.startsWith('preferences'))).toBe(true);
    });
    ...
    it('should handle multiple keywords for same topic', async () => {
      ...
      const familyTopics = result.topics.filter(t => t.topic.startsWith('family'));
      expect(familyTopics.length).toBeGreaterThanOrEqual(1);
    });

    it('should detect multiple different topics', async () => {
      service.configure({ maxTopics: 5 });
      const result = await service.detect('My wife has a meeting at the gym today');
    ...
    it('should boost parent topics when child matches', async () => {
      ...
      const hasImmediate = result.topics.some(t => t.topic === 'family/immediate');
  ...
  describe('topic scoring', () => {
    ...
    it('should respect maxTopics limit', async () => {
      ...
      const result = await service.detect('My wife has a meeting at the gym today for the project');
    ...
    it('should sort topics by confidence descending', async () => {
      ...
      if (result.topics.length > 1) {
        for (let i = 0; i < result.topics.length - 1; i++) {
          expect(result.topics[i].confidence).toBeGreaterThanOrEqual(result.topics[i + 1].confidence);
        }
      }
    });
  });
  ...
  describe('topic history tracking', () => {
    it('should track topics by user', async () => {
      await service.detect('family', { userId: 'user1', recentTopics: [], recentMessages: [] });
      await service.detect('work', { userId: 'user1', recentTopics: [], recentMessages: [] });
  ...
  describe('topic shift detection', () => {
    it('should detect topic shift when topics change', async () => {
      ...
      await service.detect('wife', { userId, recentTopics: [], recentMessages: [] });
      await service.detect('daughter', { userId, recentTopics: [], recentMessages: [] });
      
      // Now switch to work topic
      const workResult = await service.detect('project deadline', { userId, recentTopics: [], recentMessages: [] });
    ...
    it('should return null when no shift', async () => {
      ...
      await service.detect('family', { userId, recentTopics: [], recentMessages: [] });
      const result = await service.detect('family', { userId, recentTopics: [], recentMessages: [] });

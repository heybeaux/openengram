...
describe('GitHubSignalService', () => {
  ...
  describe('when configured', () => {
    ...
    it('should collect commit observations', async () => {
      const mockCommits = [
        {
          sha: 'abc',
          commit: {
            author: { name: 'Dev', date: '2026-02-22T00:00:00Z' },
      ...
      expect(result.observations[0].content).toContain(
        '1 commits to owner/repo-a',
      );
    ...
    it('should detect stale PRs', async () => {
      ...
      jest
        ...
        .mockResolvedValueOnce({
          ok: true,
          json: () => Promise.resolve([]),
        } as any); // repo-b issues

      const result = await service.collect(null, { maxQueries: 10 });
      const prObs = result.observations.find(
        (o) => o.metadata?.type === 'open_prs',
      );
    ...
    it('should filter PRs from issues endpoint', async () => {
      ...
      jest
        ...
        .mockResolvedValueOnce({
          ok: true,
          json: () => Promise.resolve([issue, prAsIssue]),
        } as any)
        .mockResolvedValueOnce({
          ok: true,
          json: () => Promise.resolve([]),
      ...
      const result = await service.collect(null, { maxQueries: 10 });
      const issueObs = result.observations.find(
        (o) => o.metadata?.type === 'closed_issues',
      );

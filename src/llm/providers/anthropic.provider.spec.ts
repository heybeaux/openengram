...
const anthropicResponse = (text: string, model = 'claude-3-5-sonnet-20241022') => ({
  content: [{ text }],
  model,
...
describe('AnthropicProvider', () => {
  ...
  describe('chat', () => {
    ...
    beforeEach(() => {
      provider = new AnthropicProvider({ apiKey: 'sk-test' });
    });
    ...
    it('should set required Anthropic headers', async () => {
      mockFetch.mockResolvedValue(
        buildResponse(anthropicResponse('Hi')),
      );
    ...
    it('should separate system message from chat messages', async () => {
      mockFetch.mockResolvedValue(
        buildResponse(anthropicResponse('Hello')),
      );
    ...
    it('should handle messages with no system message', async () => {
      mockFetch.mockResolvedValue(
        buildResponse(anthropicResponse('Response')),
      );
    ...
    it('should use max_tokens from options', async () => {
      ...
      await provider.chat([{ role: 'user', content: 'Hi' }], { maxTokens: 1024 });
    ...
    it('should handle empty content array gracefully', async () => {
      mockFetch.mockResolvedValue(
        buildResponse({ content: [], model: 'claude-3-5-sonnet', usage: { input_tokens: 10, output_tokens: 0 } }),
      );
  ...
  describe('json', () => {
    ...
    it('should extract JSON from markdown code block', async () => {
      ...
      mockFetch.mockResolvedValue(
        buildResponse(anthropicResponse(mdResponse)),
      );

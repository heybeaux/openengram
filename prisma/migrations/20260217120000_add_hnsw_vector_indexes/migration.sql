...
CREATE INDEX CONCURRENTLY IF NOT EXISTS "memory_embeddings_embedding_hnsw_idx"
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS "memory_embeddings_bge_base_hnsw_idx"
ON "memory_embeddings" USING hnsw (embedding vector_cosine_ops)
WITH (m = 16, ef_construction = 64)
WHERE model_id = 'bge-base' AND embedding IS NOT NULL;

CREATE INDEX CONCURRENTLY IF NOT EXISTS "memory_embeddings_minilm_hnsw_idx"
ON "memory_embeddings" USING hnsw (embedding vector_cosine_ops)
WITH (m = 16, ef_construction = 64)
WHERE model_id = 'minilm' AND embedding IS NOT NULL;

CREATE INDEX CONCURRENTLY IF NOT EXISTS "memory_embeddings_nomic_hnsw_idx"
ON "memory_embeddings" USING hnsw (embedding vector_cosine_ops)
WITH (m = 16, ef_construction = 64)

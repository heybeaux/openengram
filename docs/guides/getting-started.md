...
This starts PostgreSQL (with pgvector), Elasticsearch, and the Engram API on `http://localhost:3001`. Interactive docs are available at `http://localhost:3001/api-docs`. Protected endpoints need an agent API key generated by registration/setup, unless you explicitly enable `TRUST_LOCAL_NETWORK=true` for trusted local development.
...
API keys are generated by the account setup/register flow and stored hashed in the database. For trusted local development only, you can enable unauthenticated local/LAN requests:

```env
TRUST_LOCAL_NETWORK=true
```

Do not enable LAN bypass behind a public reverse proxy or on an untrusted network.
